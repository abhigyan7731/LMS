...
interface CourseEditorProps {
  course: { id: string; title: string; description: string | null; price: number; is_published: boolean };
  chapters: Chapter[];
}

export function CourseEditor({ course, chapters: initialChapters }: CourseEditorProps) {
  ...
  const saveOrder = async (order: Chapter[]) => {
    setSaving(true);
    try {
  ...
  const updateChapter = async () => {
    ...
    try {
      ...
      setChapters((prev) =>
        prev.map((c) => (c.id === selectedChapter.id ? { ...c, title, description } : c))
      );
                ...
                .map((ch, i) => (
                  <div
                    ...
                    className={`flex items-center gap-2 p-3 cursor-pointer group ${
                      selectedChapter?.id === ch.id ? 'bg-primary/10' : 'hover:bg-accent/50'
                    }`}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {ch.mux_playback_id && (
                        <Video className="h-3 w-3 text-primary inline mr-1" />
                      )}
                    </div>
                    <Button
                      size="icon"
              ...
              <div>
                ...
                {selectedChapter.mux_playback_id && (
                  <p className="text-xs text-muted-foreground mt-2">
                    Video: {selectedChapter.mux_playback_id}
                  </p>
                )}
              </div>
              <Button onClick={updateChapter} disabled={saving}>
                Save Chapter
              </Button>
            </CardContent>
          </Card>
