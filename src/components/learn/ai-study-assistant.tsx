  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: 'Sorry, I could not process your question.' },
        ]);
      }
    } catch {
      setMessages((prev) => [
    ...
    <Card glass>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Ask questions about &quot;{chapterTitle}&quot;. Answers are based on the chapter content.
        </p>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-4 max-h-[400px] overflow-y-auto mb-4">
          {messages.length === 0 && (
            <p className="text-sm text-muted-foreground">
              Ask anything about this chapter. The AI uses the transcript to answer.
            </p>
          )}
          {messages.map((m, i) => (
            <div
